...
project_dir <- "/lustre/scratch125/casm/team113da/projects/FUR/FUR_analysis/FUR_analysis_cat/pathogen_identification/analysis/7097_3073"
...
krakentools_files <- fs::dir_ls(path = project_dir,
                      type = "file",
                      glob = "*.mpa$",
                      recurse = TRUE)
...
cohort_analysis <- function(x){
    ...
    filtered_df <- x |> filter(rank %in% c("S","G","F"))
